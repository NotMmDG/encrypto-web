...
from app.utils.security import verify_password, decrypt_file, huffman_decompress  # <-- added import
...
async def download_file(
    ...
    if not verify_password(password, db_file.password):
        # No logging of plaintext/hashed passwords
    ...
    # If it looks like a Huffman blob, decompress
    if decrypted_content.startswith(b"HUFF1"):
        try:
            decrypted_content = huffman_decompress(decrypted_content)
        except Exception:
            # Corrupted or unexpected format
            raise HTTPException(status_code=500, detail="Failed to decompress file content")
